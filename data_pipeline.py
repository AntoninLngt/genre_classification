...
def get_dataset(input_csv, batch_size=8):
    ...
    dataset = dataset.map(lambda sample: dict(sample, mfcc=mfcc(sample["waveform"])))
    ...
    dataset = dataset.map(lambda sample: (sample["waveform"], sample["one_hot_label"],sample["zcr"],sample["centroid"],sample["mfcc"]))
